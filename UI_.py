...
class UI(QWidget) :
    ...
    def __init__(this , *args , **kwargs) :
        ...
        if not load(open("_config_.json"))["OS_checked"] :
            ...
            system("python OS_detector.py")
            this.__update_config("OS_checked",True)
        ...
        this.language.setCurrentText(load(open("_config_.json"))["current_language"])
        this.language.currentTextChanged.connect(lambda : this.__update_config("current_language" , this.language.currentText()))
        ...
        this.accent.addItems(load(open("_config_.json"))["accent"])
        ...
        this.accent.setCurrentText(load(open("_config_.json"))["current_accent"])
        this.accent.currentTextChanged.connect(lambda : this.__update_config("current_accent" , this.accent.currentText()))
        ...
        this.save_loc_box.setText(load(open("_config_.json"))["save_location"] )
        this.save_loc_box.textChanged.connect(lambda: this.__update_config("save_location",this.save_loc_box.text()))
    ...
    def __menubar_setup(this) :
        ...
        about.triggered.connect(lambda : this.__message_display("About" , load(open("_config_.json"))["about_message"] ) )
    ...
    def __update_config(this,key : str , value : str | list) :
    ...
    def __play_pressed(this) :
        x = create_audio(this.textbox.toPlainText() , this.language.currentText() , this.accent.currentText() , this.save_loc_box.text() , this.speed_checkbox.isChecked())
        if x.__len__() != 0 :
